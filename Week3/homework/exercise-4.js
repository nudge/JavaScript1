...
function addToShoppingCart(item) {
  ...
  if (listOfGroceryItems.length > 3) {
    listOfGroceryItems.shift();
  }
  return listOfGroceryItems;
}
let listOfGroceryItems = ['bananas', 'milk'];
addToShoppingCart('apple');
addToShoppingCart('grapes');
addToShoppingCart('watermeloen');
console.log(listOfGroceryItems);
