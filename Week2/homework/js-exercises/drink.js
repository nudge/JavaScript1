...
for(let i=0; i<5; i++){
    while(true){
        var randomItem = drinkTypes[Math.floor(Math.random()*drinkTypes.length)]; // Getting a random item from drinkTypes array
        ...
        if(drinkTray.filter(item => { return item === randomItem }).length<2) //
        {
